...
export const generateProposal = async (data: ProposalData): Promise<string> => {
  ...
  const userPrompt = `
客戶資訊: { 姓名: "${data.client.name}", 年齡: ${data.client.age} }
計劃名稱: "${data.planName}"
保費數據: { 總額: ${data.premium.total}, 繳費方式: "${data.premium.paymentType}" }
情境A數據:
...
情境B數據:
  ...
  第10年: { 累計提取: ${data.scenarioB.year10.cumulative}, 剩餘價值: ${data.scenarioB.year10.remaining} }
  第20年: { 累計提取: ${data.scenarioB.year20.cumulative}, 剩餘價值: ${data.scenarioB.year20.remaining} }
  第30年: { 累計提取: ${data.scenarioB.year30.cumulative}, 剩餘價值: ${data.scenarioB.year30.remaining} }
  第40年: { 累計提取: ${data.scenarioB.year40.cumulative}, 剩餘價值: ${data.scenarioB.year40.remaining} }
推廣優惠: 
  回贈: "${rebateString}"
  預繳利率: "${prepayString}"
`;
